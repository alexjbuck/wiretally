...
```bash
net-counter --domain-prefix amazonaws.com -- mcap info s3://my-bucket/dataset.mcap
```
...
```text
net-counter [OPTIONS] -- <COMMAND> [ARGS...]
...
The child's stdout and stderr are inherited directly, and `net-counter` exits with the child's
...
```bash
net-counter -j -d amazonaws.com -- mcap info s3://bucket/file.mcap \
  | jq '.endpoints[] | select(.matches_filter) | {domain, ingress_bytes}'
```

## How it works

1. An HTTP proxy binds an OS-assigned port on `127.0.0.1`.
2. `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` (and their lowercase spellings) are set in the
   child's environment only — nothing global is touched.
3. `CONNECT host:port` requests are answered with `200 Connection Established` and then spliced
   as raw TCP. TLS is never terminated: no certificate to install, no crypto on the data path,
   and the counters see exactly the ciphertext that crossed the wire. Plain HTTP requests are
...
- Clients that ignore the proxy environment variables are not measured. That covers raw sockets,
  most gRPC clients, DNS itself, and anything with `NO_PROXY` set for the host in question.
- QUIC / HTTP-3 traffic bypasses an HTTP proxy entirely; a client that negotiates it will show
  up only as the fallback TCP connections it makes, if any.
- The `CONNECT` handshake between child and proxy is excluded from the totals — it is local
